            ...
            <ul class="nav user-menu">
                ...
                <li class="nav-item dropdown">
                    <a href="#" class="dropdown-toggle nav-link" data-toggle="dropdown">
                        ...
                        <span class="badge badge-pill">{{ $unreadCount ?? 0 }}</span>
                    </a>
                        ...
                        <div class="noti-content">
                            <ul class="notification-list">
                                @forelse($notifications as $notification)
                                    <li class="notification-message {{ $notification->is_read ? '' : 'unread' }}">
                                        <a href="{{ $notification->url ?? '#' }}">
                                            <div class="media">
                                                <span class="avatar">
                                                    <img alt="User Avatar"
                                                        src="{{ URL::to('/assets/images/' . ($notification->user->avatar ?? 'default.png')) }}">
                                                </span>
                ...
                <li class="nav-item dropdown">
                    <a href="#" class="dropdown-toggle nav-link" data-toggle="dropdown">
                        ...
                        <span class="badge badge-pill">{{ $messageUnreadCount ?? 0 }}</span>
                    </a>
                    <div class="dropdown-menu notifications">
                        <div class="topnav-dropdown-header">
                            <span class="notification-title">Messages</span>
                            <a href="{{ route('messages.markAllRead') }}" class="clear-noti">Clear All</a>
                        </div>
                        <div class="noti-content">
                            <ul class="notification-list">
                                @forelse($messages as $message)
                                    <li class="notification-message {{ $message->is_seen ? '' : 'unread' }}">
                                        <a href="{{ route('chat', $message->sender->id) }}">
                                            <div class="list-item">
                                                <div class="list-left">
                                                    <span class="avatar">
                                                        <img alt=""
                                                            src="{{ URL::to('/assets/images/' . ($message->sender->avatar ?? 'default.png')) }}">
                                                    </span>
                                                </div>
                                                <div class="list-body">
                                                    <span class="message-author">{{ $message->sender->name }}</span>
                                                    <span
                                                        class="message-time">{{ $message->created_at->diffForHumans() }}</span>
                                                    <div class="clearfix"></div>
                                                    <span
                                                        class="message-content">{{ Str::limit($message->body, 50) }}</span>
                                                </div>
                                            </div>
                                        </a>
                                    </li>
                        ...
                        <div class="topnav-dropdown-footer">
                            <a href="{{ route('chat', Auth::id()) }}">View all Messages</a>
                        </div>
                    </div>
                </li>
