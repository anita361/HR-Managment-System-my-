    ...
    $.ajax({
        ...
        success: function(messages) {
            ...
            messages.forEach(function(msg) {
                ...
                let checkbox = `<input type="checkbox" class="forward-checkbox" value="${msg.id}" style="margin-right:5px;">`;

                let content = `<div style="position:relative;padding-right:30px;">`;

                // Chat menu button
                content += `<div style="position:absolute;top:0;right:0;">`;
                content += `<button onclick="toggleMenu(${msg.id})" style="background:none;border:none;font-size:18px;cursor:pointer;">⋮</button>`;
                content += `<div id="menu-${msg.id}" class="chat-menu" style="display:none;position:absolute;right:0;top:22px;background:#fff;
                             border:1px solid #ddd;border-radius:4px;box-shadow:0 2px 6px rgba(0,0,0,0.15);z-index:100;">`;

                if (msg.sender_id === myId) {
                    // Sent messages → Edit, Forward, Delete for me, Delete for everyone
                    content += `<div onclick="startEditMessage(${msg.id}, \`${msg.body ?? ''}\`)" style="padding:8px 12px;cursor:pointer;">✏️ Edit</div>`;
                    content += `<div onclick="forwardMessage(${msg.id})" style="padding:8px 12px;cursor:pointer;">📤 Forward</div>`;
                    content += `<div onclick="deleteMessage(${msg.id}, false)" style="padding:8px 12px;color:red;cursor:pointer;">🗑 Delete for me</div>`;
                    content += `<div onclick="deleteMessage(${msg.id}, true)" style="padding:8px 12px;color:red;cursor:pointer;">🗑 Delete for everyone</div>`;
                } else {
                    // Received messages → Delete for me only
                    content += `<div onclick="deleteMessage(${msg.id}, false)" style="padding:8px 12px;color:red;cursor:pointer;">🗑 Delete for me</div>`;
                }
                ...
                // Message content
                if (msg.is_deleted) {
                    content += `<p style="font-style:italic;color:#888;">🚫 This message was deleted</p>`;
                } else {
                    if (msg.body) content += `<p style="margin:0 0 5px;">${msg.body}</p>`;
                    if (msg.file) content += `<p style="margin:0;"><a href="/assets/images/${msg.file}" target="_blank">${msg.file}</a></p>`;
                }
                ...
                // Append to chat
                if (msg.sender_id === myId) {
                    let seenStatus = msg.is_seen == 1 ? '<small class="text-primary">✔✔ Seen</small>' : '<small class="text-muted">✔ Sent</small>';
                    chatBox.append(`
                        <div class="chat chat-right">
                            <div class="chat-avatar">
                                <a href="#" class="avatar"><img src="${senderAvatar}" alt="You"></a>
                            </div>
                            <div class="chat-body">
                                <div class="chat-bubble">
                                    <div class="chat-content">
                    ...
                    chatBox.append(`
                        <div class="chat chat-left">
                            <div class="chat-avatar">
                                <a href="#" class="avatar"><img src="${senderAvatar}" alt="${msg.sender.name}"></a>
                            </div>
                            <div class="chat-body">
                                <div class="chat-bubble">
                                    <div class="chat-content">
...
window.deleteMessage = function(id, forEveryone = false) {
    if (forEveryone) {
        ...
        if (!confirm('Delete message for me?')) return;
    }
    ...
    $.ajax({
        ...
        success: function(res) {
            if (res.status) {
                alert(forEveryone ? 'Message deleted for everyone' : 'Message deleted for me');
                fetchMessages();
            }
        },
        error: function(xhr) {
